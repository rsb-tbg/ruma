...
/// A to-device `m.key.verification.request` event when pushed to an appservice.
pub type AppserviceToDeviceKeyVerificationRequestEvent =
    AppserviceToDeviceEvent<ToDeviceKeyVerificationRequestEventContent>;

/// The content of a to-device `m.key.verification.request` event when pushed to an appservice.

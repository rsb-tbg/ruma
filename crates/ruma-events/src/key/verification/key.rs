...
/// A to-device `m.key.verification.key` event when pushed to an appservice.
pub type AppserviceToDeviceKeyVerificationKeyEvent =
    AppserviceToDeviceEvent<ToDeviceKeyVerificationKeyEventContent>;

/// The content of a to-device `m.key.verification.key` event when pushed to an appservice.

...
pub type AppserviceToDeviceRoomKeyEvent = AppserviceToDeviceEvent<ToDeviceRoomKeyEventContent>;
pub type AppserviceToDeviceRoomKeyEventContent = ToDeviceRoomKeyEventContent;
...
// /// The content of an `m.room_key` appservice event.
// ///
// /// Typically encrypted as an `m.room.encrypted` event, then sent as an appservice to-device
// event. #[derive(Clone, Debug, Deserialize, Serialize, EventContent)]
// #[cfg_attr(not(ruma_unstable_exhaustive_types), non_exhaustive)]
// #[ruma_event(type = "m.room_key", kind = AppserviceToDevice)]
// pub struct AppserviceToDeviceRoomKeyEventContent {
//     /// The encryption algorithm the key in this event is to be used with.
//     ///
//     /// Must be `m.megolm.v1.aes-sha2`.
//     pub algorithm: EventEncryptionAlgorithm,
...
// impl AppserviceToDeviceRoomKeyEventContent {
//     /// Creates a new `AppserviceToDeviceRoomKeyEventContent` with the given algorithm, room ID,
